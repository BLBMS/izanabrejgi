...
console.log('Script.js se nalaga...');

// ============ INICIALIZACIJA EVENT LISTENERJEV ============
document.addEventListener('DOMContentLoaded', function () {
    console.log('DOMContentLoaded - inicializiram event listenerje za OPNsense');

    // Event listenerji za navigacijske gumbe
    document.getElementById('nav-home').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na Domov');
        showHome();
    });

    document.getElementById('nav-description').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na Opis');
        showDescription();
    });

    document.getElementById('nav-about').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na Ponujamo');
        showAbout();
    });

    document.getElementById('nav-pricing').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na Cenik');
        showPricing();
    });

    document.getElementById('nav-contact').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na Kontakti');
        showContact();
    });

    // Event listenerji za slideshow gumbe
    document.getElementById('prev-slide').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na prev slide');
        prevSlide();
    });

    document.getElementById('next-slide').addEventListener('click', function (e) {
        e.preventDefault();
        console.log('Klik na next slide');
        nextSlide();
    });

    // Prepreči default action za anchorje z #
    ...
    // Inicializiraj ostale stvari
    setTimeout(() => {
        createSlideshowDots();
    }, 500);

    // Inicializiraj jezikovne zastavice
    setTimeout(initLanguageFlags, 100);

    // Naloži jezikovne podatke
    setTimeout(loadLanguageData, 200);

    console.log('Event listenerji inicializirani');
});

// ============ OSTALE FUNKCIJE IZ ORIGINALNEGA SCRIPT.JS ============
// Prekopiraj vse ostale funkcije iz tvojega obstoječega script.js tukaj
// (shuffleArray, showNextSlide, hideAllOverlays, showDescription, showAbout,
//  showContact, showPricing, showHome, loadSlides, prevSlide, nextSlide, itd.)
...
// GLOBALNA FUNKCIJA ZA SKRIVANJE VSEH OVERLAYEV
function hideAllOverlays() {
    // 1. Skrij overlay ozadje
    const bg = document.getElementById('overlay-background');
    if (bg) {
        bg.classList.remove('active');
    }

    // 2. Skrij VSE overlaye - DODAL "#direct-booking-overlay"
    const allOverlayElements = document.querySelectorAll(
        '.text-overlay, .contact-overlay, #pricing-overlay, #description-overlay, #about-overlay, #contact-overlay, #direct-booking-overlay'
    );
...
document.addEventListener('click', function (event) {
    ...
    if (target.id === 'overlay-background') {
        hideAllOverlays();
    }
});
...
// Funkcija za mobilni spustni meni
...
async function loadPricingData() {
    try {
        ...
        document.getElementById('pricing-table').innerHTML =
            '<tr><td colspan="4" style="color: var(--font-color); text-align: center; padding: 40px;">Cenik trenutno ni na voljo. Prosimo, poskusite kasneje.</td></tr>';
    }
}
...
// Funkcija za določanje trenutnega jezika
...
function applyLanguage(lang) {
    ...
    document.getElementById('nav-direct').textContent = data.nav.direct;
...
/*                                      function showDirectBooking() {
                                            hideAllOverlays();
                                            const bg = document.getElementById('overlay-background');
                                            const directOverlay = document.getElementById('direct-booking-overlay');
                                            if (bg) bg.classList.add('active');
                                            if (directOverlay) directOverlay.classList.add('active');
                                            window.scrollTo({ top: 0, behavior: 'smooth' });
                                        }
...
// ============ FUNKCIJA ZA NEPOSREDNO REZERVACIJO ============
function showDirectBooking() {
    console.log('showDirectBooking() klicana - odpiram overlay');
    hideAllOverlays();
    ...
    if (bg) {
        ...
        console.log('Background overlay aktiviran');
    }

    if (directOverlay) {
        directOverlay.classList.add('active');
        console.log('Direct booking overlay aktiviran');
    } else {
        console.error('Direct booking overlay ni najden!');
    }

    window.scrollTo({ top: 0, behavior: 'smooth' });
}

console.log("Script.js naložen!");
